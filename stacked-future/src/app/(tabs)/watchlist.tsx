      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-4 mb-3" contentContainerStyle={{ gap: 6 }}>
        {CATEGORIES.map((c) => (
          <Pressable
            ...
            className={`px-3 py-1.5 rounded-xl border ${category === c ? "bg-card2 border-border2" : "border-border bg-transparent"}`}
          >
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          {displayList.map((item) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => { setActiveSymbol(item.symbol); router.replace("/"); }}
              >
          ...
          {!isPro && watchlist.length > FREE_LIMIT && (
            <View className="mx-4 mt-3 bg-card rounded-xl p-4 border border-border items-center">
              ...
              <Text className="text-muted text-xs text-center mb-3">Upgrade to Pro for unlimited watchlist symbols.</Text>
